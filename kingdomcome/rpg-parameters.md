...
BaseAttackStaminaCost | how much stamina will attack cost
...
BestVisVolume | object volume for maximum recognition bonus
...
BowChargeDurationMin | minimum duration of bow charge animation
...
CaffeineFromFoodCoef | how much caffeine is added from a unit refresh
CaffeineFullThreshold | you cannot use more refreshing items if above this value
...
CombatAutoClinchReactionDelayMaxMax | maximal maximum
CombatAutoClinchReactionDelayMaxMin | maximal minimum
...
CombatAutoMaxAttackDelay | mean delay between attacks for a defensive AI
...
CombatAutoZoneChangeDelayMinMin | zone change timeout minimal minimum
...
DawnTime | Time when night ends, used by rpg (default 04:30)
...
DefaultVisVolume | default item size-volume that have no visibility recognition penalty
...
EncumberedToSpeedSurfaceCoef | Coef that controls terrain/surface influence in encumbered speed mod
...
ExtremeExhaustionFaintAveragePeriod | How often (on average, in real time) should player faint when extremely exhausted (exhaust stat is on 0).
...
FatalFallHeight | falling height above which fatal health damage is taken at agility 0
...
FoodPoisoningThreshold | threshold for food posioning to start taking affecting
...
FoodWitcherPerkNutritionModif | potion nutrition wicher perk modif
ForcedFireAimSpreadMalus | spread added when the rpg forces the firing on low stamina
FromBehindAngle | minimal angle to classify the attack as 'from behind' - 0 face, PI back
FullClothDirtyingOnFullSpeed | how far do we walk with full speed (1.0 walk speed) to get 100% dirty
...
HealthDeltaAbsLimit | abs max of health delta
...
HorseRidingToHorseCourage | how the rider's horse riding skill increases courage of his horse
...
LockpickingFailSoundIntensity | one-shot intensity relative to the database
LockpickingLockpickBreakChance | base lockpick break chance
LockpickingRelDistanceToSoundIntensity | how relative distance influences sound
LockPickingSkillToShakeRel | how much does the skill decrease the cursor shake (skill * this is relative to maximum/base)
...
LockPickingToleranceACoef | the lockpicking tolerance formula
LockPickingToleranceKCoef | the lockpicking tolerance formula
LockPickingToleranceMCoef | the lockpicking tolerance formula
LockPickingToleranceNCoef | the lockpicking tolerance formula
...
MaxStealthHitSoundMultiplier | intensity multiplier for max stealth level
...
MinPedalCost | pedaling STA cost (pressure 0)
...
PicklockDmgSpeed | how fast is picklock durability decreasing (will be multiplied by the relative distance)
...
PickpocketingMaxSkillChargeTime | max charge time with best skill
...
RangedWpnMinStrCoef | if the curr/req strength ratio goes below this, the power is minimal
RangedWpnOptimalDistanceToMinamal | ratio between the database attack distance and the minimal range for the AI
RangedWpnPowerConstA | used to convert strength requirements to the resulting power
RangedWpnPwrToSpeed | total power to launch speed
RangedWpnSelfHarmCoef | Special constant used in self harm equation.
RangedWpnSpeedToAttack | attack mod deduced from impact speed
ReadingRestEffectiveness | If this value is 0.3, reading will regen player as sleeping on bed with comfort 30%.
...
ReadingXpPerHour | how much xp in reading skill player gets after one hour of reading with reading speed 100%
...
RecognitionTimeKNegativeCoef | multiplicative coef for negative values of conspicuousness
RecognitionTimeKPositiveCoef | multiplicative coef for positive values of conspicuousness
...
RespawnTimeBase | time before a hidden corpse respawns (base time)
...
SkillXPKill | weapon XP gain after a kill
...
StaminaToHealthDamageMax | stamina-health damage transfer for dmgr=1
StaminaToHealthDamageMin | stamina-health damage transfer for dmgr=0
...
StealthSkillToViewRadiusDecr | how much is the view radius decreased by the skill level (relative)
StealthSneakBaseDistance | sneaked distance that triggers stealth leveling
...
TrueRelationDistThresholdRel | distance, relative to observers maximum distance, required to see the true faction
...
UnconsciousDepthFadeoutSpeedBase | how fast is the depth consumed
