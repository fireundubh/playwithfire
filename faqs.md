...
Body | Be specific, concise, and include only relevant information.<br><br>Use the following structure for reporting issues:<br><br>CONDITIONS<br>The conditions describe the state of the operating environment at the time the issue occurred. Use only when needed.<br><br>STEPS TO REPRODUCE<br>The steps to reproduce describe the procedure by which any other user can experience the same issue.<br><br>Use only when reproducing the issue has either:<br>-	a frequency lower than 100%, or<br>-	when reproduction is sufficiently complex to warrant instructions.<br><br>RESULT<br>The result describes what actually happened.<br><br>This is the only section that is required.<br><br>Do not use a heading when reporting only the result.<br><br>EXPECTED RESULT<br>The expected result describes what should have happened.<br><br>Use only when the expected result is not obvious.<br><br>Always use headings when including two or more sections.